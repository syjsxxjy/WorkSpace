...
public class TilesPage extends Keys{
	/*
	 ...
	 */
	   public  StringBuilder getPage(Map<String, String> map,Map<String, String> mapcn){//map：英文键值对   mapcn：中文键值对
		   ...
		   if(map.containsKey("materialName")){
			   itemid=map.get("materialName").replace(" ", "");   System.out.println(itemid);  
		   }else if(map.containsKey("modName")){
			   itemid=map.get("modName").replace(" ", "");   System.out.println(itemid);  
		   }
		      //设定文件名中文部分
		       if( mapcn.get("shortdescription")!=null){
		    	   cnname=mapcn.get("shortdescription").replace("。", "");   //System.out.println(cnname);
	       }else{
	    	   cnname=" ";}
		       //设定物品英文名，因为mod物品没有shortdescription属性，所以以mod那么来作为物品英文名称
		       if(map.containsKey("shortdescription")){
		    	   enname=map.get("shortdescription");
		       }else if(map.containsKey("modName")){
		    	   enname=map.get("modName");
		       }
		       //设定中文物品描述，可能为空字符
		      ...
		      //原游戏图片路径参数预处理
		       if(map.get("texture")!=null){
			       ...
			       Matcher m = p.matcher(map.get("texture"));  
			       while(m.find()){
			     ...
			     //wiki图片存放路径,只有文件名，不带路径，路径在filecopy时创建 为每一个物品ID创建一个单独的路径存放文件  
			       ...
			       Matcher m2 = p2.matcher(map.get("texture"));  
			       while(m2.find()){
			...
			//↑此处若存在中文名则需要在最后加上冒号
			sb.append("^{{::starbound:tiles:"+itemid+":"+wikiinventoryIconpath+"?100|}}|^"+"\r\n");  //这里的数字为图片大小，需要测试之后再设定
			//↑tiles 的图片文件由于会冲突，所以再新键一个文件夹tiles，把所有子文件夹放在tiles目录下
			sb.append("^  物品ID：|**"+itemid+"**|"+"\r\n");
			sb.append("^  物品名称：|"+cnname+" "+enname+"|"+"\r\n");
			sb.append("^  价格：|"+map.get("price")+"|"+"\r\n");
			sb.append("^  物品描述：|"+cndescription+""+map.get("description")+"|"+"\r\n");
			//以下是可能存在的属性--------------------------------------------
			sb.append("|  稀有程度：|"+raritycn+"|"+"\r\n");
			if (map.containsKey("modId")){  sb.append("|  ModID：|"+map.get("modId")+"|"+"\r\n");}
			if (map.containsKey("itemDrop")){  sb.append("|  物品掉落：|"+map.get("itemDrop")+"|"+"\r\n");}
			if (map.containsKey("platform")){  sb.append("|  平台：|"+map.get("platform")+"|"+"\r\n");}
