...
public class TechPage extends Keys{
	/*
	 ...
	 */
	   public  StringBuilder getPage(Map<String, String> map,Map<String, String> mapcn){//map：英文键值对   mapcn：中文键值对
	    	  //设定itemid
		      itemid=map.get("itemName").replace(" ", "");   System.out.println(itemid);  
		      //设定文件名中文部分
		      ...
		      //原游戏图片路径参数预处理
		       if(map.get("inventoryIcon")!=null){
			       ...
			       Matcher m = p.matcher(map.get("inventoryIcon").replace("[", ""));  
			       while(m.find()){
			     ...
			     //wiki图片存放路径,只有文件名，不带路径，路径在filecopy时创建 为每一个物品ID创建一个单独的路径存放文件  
			       ...
			       Matcher m2 = p2.matcher(map.get("inventoryIcon").replace("[", ""));  
			       while(m2.find()){
            ...
            if (map.containsKey("ceiling")){  sb.append("|  是否悬挂：|"+ceilingcn+"|"+"\r\n");}
            
            
            
            if (map.containsKey(" floranDescription")){sb.append("|  种族描述  ||"+"\r\n");}
